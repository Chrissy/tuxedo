...
(function() {
  window.Image = (function() {
    ...
    Image.prototype.dimension = function(name) {
      var dimension, self;
      dimension = this.src.match(RegExp("\\&" + name + "\\=(.*?)(\\&|$)"))[1];
      self = this;
      return {
        increment: function() {
    ...
    Image.prototype.filepicker_url = function() {
      var url;
      url = this.src.match(/\/file\/(.*?)\//)[1];
      return "https://www.filepicker.io/api/file/" + url + "/convert?fit=crop&h=" + (this.dimension("h").increment()) + "&w=" + (this.dimension("w").increment()) + "&cache=true";
    };

    Image.prototype.upscale = function() {
      var img, self;
      self = this;
      return img = $("<img/>").attr("src", this.filepicker_url()).load(function() {
...
(function() {
  window.Form = (function() {
    function Form(form) {
      ...
      self = this;
      this.autocomplete = [];
      if (this.form.hasClass("components")) {
        this.generateAutocomplete("/ingredients/all.json", ":");
      }
      if (this.form.hasClass("recipes")) {
        this.generateAutocomplete("/all.json", "=");
      }
      if (this.form.hasClass("lists")) {
        this.generateAutocomplete("/list/all.json", "#");
      }
    }

    Form.prototype.getElements = function(url) {
      return $.get(url).then(function(elements) {
        return elements;
      }, function() {
    ...
    Form.prototype.generateAutocomplete = function(url, flag) {
      var self;
      self = this;
      return this.getElements(url).promise().then(function(data) {
        return self.setupAutoComplete(data, flag);
      });
    };

    Form.prototype.setupAutoComplete = function(data, flag) {
      var self;
      self = this;
      return this.autocomplete[flag] = this.form.atwho({
        at: flag,
        data: data
      }).on("inserted.atwho", function(event, flag, query) {
        flag = flag.text().trim();
        return self.swapWithComponentLink(query, flag);
      });
    };

    Form.prototype.swapWithComponentLink = function(query, flag) {
      var aftertext, newstr, pretext;
      pretext = this.form.val().substring(0, query.pos);
      aftertext = this.form.val().substring(query.pos + flag.length + 2);
      newstr = "" + pretext + "[" + flag + "] " + aftertext;
      return this.form.val(newstr);
    };
...
(function() {
  window.Search = (function() {
    ...
    Search.prototype.build_typeahead = function(bloodhound_instance) {
      return this.input.typeahead(null, {
    ...
    Search.prototype.get = function() {
      return $.get("/search.json").then(function(data) {
        return $.makeArray(data);
      });
    };

    Search.prototype.listenForSelect = function() {
      return this.input.on("typeahead:selected", function(event, object, name) {
        return window.location = object.url;
      });
    };
...
(function() {
  $(function() {
    var toggleSocialPrompt, upscale_all_images;
    window.form = new Form('.autocomplete');
    window.search = new Search('.search');
    upscale_all_images = function() {
      return $("[data-resize]").each(function() {
        return new Image($(this)).upscale();
      });
    };
    upscale_all_images();
    $(".authenticate").each(function() {
      var $this;
      $this = $(this);
      return $.get('/authenticate_before_render').promise().then(function(data) {
        if (Boolean(data)) {
          return $this.addClass("authenticated");
        }
      });
    });
    ...
    $("[data-lazy-load]").each(function() {
      return $.ajax({
        url: "/list/get/" + ($(this).attr("data-lazy-load")) + "?start=" + ($(this).find(".list-element").length),
        success: (function(_this) {
          return function(data) {
            $(_this).append(data);
            return upscale_all_images();
          };
        })(this)
      });
    });
    ...
    toggleSocialPrompt = (function(_this) {
      return function() {
        return $(".social-prompt").toggleClass("active");
      };
    })(this);
    window.setTimeout(toggleSocialPrompt, 1000);
    return window.setTimeout(toggleSocialPrompt, 4000);
  });
