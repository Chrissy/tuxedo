...
(function() {
  window.Form = (function() {
    function Form(form) {
      ...
      this.form = $(form);
      self = this;
      this.getAllComponents().promise().then(function(data) {
        return self.setupAutoComplete(data);
      });
    }

    Form.prototype.getAllComponents = function() {
      return $.get("/components/all.json").then(function(components) {
        return components;
      }, function() {
    ...
    Form.prototype.setupAutoComplete = function(data) {
      var self;
      self = this;
      return this.autocomplete = this.form.atwho({
        at: ":",
        data: data
      }).on("inserted.atwho", function(event, flag, query) {
        flag = flag.text().trim();
        self.swapWithComponentLink(query, flag);
        return self.addToComponentList(flag);
      });
    };
    ...
    Form.prototype.swapWithComponentLink = function(query, flag) {
      ...
      aftertext = this.form.val().substring(query.pos + flag.length);
      newstr = "" + pretext + "[" + flag + "] " + aftertext;
      return this.form.val(newstr);
    };

    Form.prototype.addToComponentList = function(flag) {
      var components;
      components = this.form.siblings("#components");
      return components.val(components.text() << flag << ",");
    };
...
(function() {
  $(function() {
    return window.form = new Form('#recipe_recipe');
  });
