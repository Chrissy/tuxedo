...
$(() => {
  ...
  fetch("/image-upload-token")
    .then(response => response.json())
    .then((response, reject) => {
      if (reject) console.warn("something went wrong", reject);
      ...
      $(".upload-cover-photo").on("change", function() {
        ...
        uploads.push(
          uploader.upload($(this).get(0).files[0]).then(file => {
            $(this)
              .siblings(".image-filename")
              .val(file.name);
          })
        );
