...
export default class Search {
  constructor(input) {
    ...
    this.searchClient = algoliasearch(
      "LD61RF5HKX",
      "7fe6167bf7f50b5582fad2c1c1797a4f"
    );
    ...
    this.get("/autocomplete.json").then((data) => {
      this.autocomplete = new Autocomplete({
        ...
        onReturnWithNoSelection: this.openFullTextSearch.bind(this),
        onFooterClick: this.openFullTextSearch.bind(this),
        limit: 6,
      });
    });
  }
  ...
  renderResults(results) {
    if (!results[0]) return "search error";

    return `
      <div class="modal__contents">
        <div class="modal__title">More results for: ${results[0].query}</div>
        ${results[0].hits
          .map((result) => this.renderSearchItem(result))
          .join("\n")}
        ${algoliaLogo}
      </div>
    `;
  ...
  renderFullTextSearch(results) {
    const r = () =>
      (this.modalContents.innerHTML = results
  ...
  getDescription(result) {
    const isQualityMatch =
      result._snippetResult && result._snippetResult.description_as_plain_text;

    return isQualityMatch
      ? result._snippetResult.description_as_plain_text.value
      : false;
  }
  ...
  getRecipe(result) {
    const isQualityMatch =
      ...
      result._snippetResult.recipe_as_plain_text &&
      result._snippetResult.recipe_as_plain_text.matchLevel === "full";

    return isQualityMatch ? result.recipe_as_plain_text.value : false;
  }

  getCount(result) {
    return result.count_for_display ? result.count_for_display : false;
  }

  renderSubtext(result) {
    const description = this.getDescription(result);
    const recipe = this.getRecipe(result);
    const count = this.getCount(result);

    return `
      ${description ? `<div class="subtext search">${description}</div>` : ""}
      ${recipe ? `<div class="subtext search">${recipe}</div>` : ""}
      ${count ? `<div class="subtext search">${count}</div>` : ""}
    `;
  }
  ...
  openFullTextSearch(value) {
    ...
    this.modal.appendChild(this.modalContents);
    this.closeSvg = document.createElement("div");
    this.closeSvg.innerHTML = closeSvg;
    this.closeSvg.classList.add("modal__close");
    this.modal.appendChild(this.closeSvg);
    this.renderFullTextSearch();
    document.body.appendChild(this.modal);
    document.querySelector(".site-container").classList.add("noscroll");

    this.searchClient
      .multipleQueries(this.getSearchQuery(value))
      .then(({ results }) => {
        this.renderFullTextSearch(results);
      });

    this.modal.addEventListener("click", (event) => {
      if (
        !this.modalContents.contains(event.target) ||
        this.modalContents === event.target
      ) {
  ...
  closeFullTextSearch() {
    ...
    this.closeSvg = null;
    document.querySelector(".site-container").classList.remove("noscroll");
  }
